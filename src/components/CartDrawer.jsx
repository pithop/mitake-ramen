            ...
            {isCartOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed top-0 right-0 h-full w-full max-w-md bg-mitake-black border-l border-white/10 shadow-2xl z-50 flex flex-col"
                    >
                        {/* Header */}
                        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-black/20">
                            <h2 className="text-2xl font-serif font-bold text-white flex items-center gap-3">
                                <ShoppingBag className="text-mitake-gold" />
                                Votre Panier
                            </h2>
                            <button
                                onClick={() => setIsCartOpen(false)}
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            {cartItems.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-gray-500 space-y-4">
                                    ...
                                    <button
                                        onClick={() => setIsCartOpen(false)}
                                        className="text-mitake-gold hover:underline"
                                    >
                                        Retourner au menu
                                    </button>
                                </div>
                                ...
                                cartItems.map((item, index) => (
                                    ...
                                    >
                                        <div className="flex-1">
                                            <h3 className="font-bold text-white text-lg">{item.titre}</h3>
                                            <p className="text-mitake-gold">{item.prix}</p>
                                            {item.kitchen_note && (
                                                <p className="text-xs text-gray-400 mt-1 italic">Note: {item.kitchen_note}</p>
                                            )}
                                        </div>
                        ...
                        {cartItems.length > 0 && (
                            <div className="p-6 border-t border-white/10 bg-black/40 space-y-4">
                                ...
                                <button
                                    ...
                                    className="w-full bg-mitake-gold text-black font-bold py-4 rounded-lg hover:bg-white transition-colors flex items-center justify-center gap-2"
                                >
                                    <Utensils size={20} />
